...
pal_scenarios <- c("red", "burlywood3", "lightsalmon", "khaki2", "lightblue", "plum", "palegreen3", "tan3", "pink", "lightgray", "wheat", "mediumpurple1", "skyblue2")
...
industry_final_en_sector_agg_global <- industry_final_en_sector_agg %>%
  group_by(Units, scenario, branch, sector_agg, year) %>%
  summarize(value = sum(value)) %>%
  ungroup()
...
```{r, message = FALSE, warning = FALSE}
ggplot(industry_final_en_sector_agg %>% filter(branch == food_branch_name & year >= 2005 & sector_agg == "food processing"),
       aes(x = year, y = value, color = scenario)) +
  geom_line(size = 1) + 
  scale_color_manual(values = pal_scenarios) + 
  facet_wrap(~region, nrow = 4, scales = "free_y") + 
  labs(x = "", y = "EJ", title = "Food processing final energy use, food processing branch") + 
  theme_bw() + 
  theme(text=element_text(size=22),
        strip.background = element_blank())
ggsave(paste0(fig_dir, "/food_pro_final_en_scenario_comp.png"), width = 30, height = 20)

ggplot(industry_final_en_sector_agg_global %>% filter(branch == food_branch_name & year >= 2005 & sector_agg == "food processing"),
       aes(x = year, y = value, color = scenario)) +
  geom_line(size = 1) + 
  scale_color_manual(values = pal_scenarios) + 
  labs(x = "", y = "EJ", title = "Global food processing final energy use, food processing branch") + 
  theme_bw() + 
  theme(text=element_text(size=17),
        strip.background = element_blank())
ggsave(paste0(fig_dir, "/food_pro_final_en_scenario_comp_global.png"), width = 10, height = 8)
```
...
food_pro_energy_inputs_edit_global <- food_pro_energy_inputs_edit %>%
  group_by(scenario, branch, sector, input_edit, Units, year) %>%
  summarize(value = sum(value)) %>%
  ungroup()

ggplot(food_pro_energy_inputs_edit %>% filter(branch == food_branch_name & year >= 2005),
       aes(x = year, y = value, fill = input_edit)) +
  geom_col() +
  facet_wrap(~scenario, ncol = 6, scale = "free_y") +
  scale_fill_manual(values = pal_sel_extended, drop = TRUE, limits = force, name = "fuel") +
  labs(x = "", y = "EJ", title = "Global food processing total energy use by fuel, food industry branch") +
  theme_bw() +
  theme(text=element_text(size=17),
        strip.background = element_blank())
ggsave(paste0(fig_dir, "/food_pro_total_energy_inputs_scenario_comp_global.png"), width = 15, height = 12)
  
ggplot(food_pro_energy_inputs_edit_global %>% filter(branch == food_branch_name & year >= 2005),
       aes(x = year, y = value, color = scenario)) +
  geom_line(size = 1) +
  scale_color_manual(values = pal_scenarios) + 
  facet_wrap(~input_edit, ncol = 4, scale = "free_y") +
  labs(x = "", y = "EJ", title = "Global food processing total energy use by fuel") +
  theme_bw() +
  theme(text=element_text(size=17),
        strip.background = element_blank())
ggsave(paste0(fig_dir, "/food_pro_total_energy_inputs_by_fuel_scenario_comp_global.png"), width = 15, height = 8)
```
...
```{r, message = FALSE, warning = FALSE}
ggplot(food_pro_process_heat_prod_tech_heat_only %>% 
         filter(branch == food_branch_name & year >= 2005),
       aes(x = year, y = value, fill = technology)) +
  geom_col() +
  facet_wrap(~scenario, ncol = 6, scale = "free_y") +
  scale_fill_manual(values = pal_sel_extended, drop = TRUE, limits = force, name = "technology") +
  ...
  theme(text=element_text(size=17),
        strip.background = element_blank())
ggsave(paste0(fig_dir, "/food_pro_process_heat_outputs_by_tech_scenario_comp_global.png"), width = 15, height = 12)

ggplot(food_pro_process_heat_prod_heat_only_total %>% filter(branch == food_branch_name & year >= 2005),
       aes(x = year, y = value, color = scenario)) +
  geom_line(size = 1) + 
  scale_color_manual(values = pal_scenarios) + 
  facet_wrap(~region, nrow = 4, scales = "free_y") + 
  labs(x = "", y = "EJ", title = "Process heat food processing production, food processing branch") + 
  theme_bw() + 
  theme(text=element_text(size=22),
        strip.background = element_blank())
ggsave(paste0(fig_dir, "/food_pro_process_heat_outputs_total_scenario_comp.png"), width = 30, height = 20)

ggplot(food_pro_process_heat_prod_heat_only_total_global %>% filter(branch == food_branch_name & year >= 2005),
       aes(x = year, y = value, color = scenario)) +
  geom_line(size = 1) + 
  scale_color_manual(values = pal_scenarios) + 
  labs(x = "", y = "EJ", title = "Global process heat food processing production, food processing branch") + 
  theme_bw() + 
  theme(text=element_text(size=17),
        strip.background = element_blank())
ggsave(paste0(fig_dir, "/food_pro_process_heat_outputs_total_scenario_comp_global.png"), width = 10, height = 8)
```
...
```{r, message = FALSE, warning = FALSE}
ggplot(CO2_emis_sector_no_bio %>% 
         filter(branch == food_branch_name & year >= 2005 & sector == "process heat food processing"),
...
```{r, message = FALSE, warning = FALSE}
ggplot(food_pro_prices %>% filter(branch == food_branch_name & year >= 2005),
       aes(x = year, y = value, color = scenario)) +
  geom_line(size = 1) + 
  scale_color_manual(values = pal_scenarios) + 
  facet_wrap(~region, nrow = 4) + 
...
ggplot(industry_final_en_sector_agg %>% filter(scenario == ref_scenario_name & year >= 2005),
  ...
  theme(text=element_text(size=17),
        strip.background = element_blank())
ggsave(paste0(fig_dir, "/industry_final_en_sector_ref_comp.png"), width = 10, height = 30)
...
industry_final_en_fuel <- industry_final_en_tech_fuel %>%
  group_by(scenario, branch, region, input, Units, year) %>%
  summarize(value = sum(value)) %>%
  ungroup() %>%
  mutate(input_edit = case_when(input == "elect_td_ind" ~ "electricity",
                                input == "refined liquids industrial" ~ "refined liquids",
...
industry_final_en_fuel_global <- industry_final_en_fuel %>%
  group_by(scenario, branch, input, input_edit, Units, year) %>%
  summarize(value = sum(value)) %>%
  ungroup()
...
ggplot(CO2_emis_sector_no_bio_industry_agg %>% filter(scenario == ref_scenario_name & year >= 2005),
       aes(x = year, y = Mt_CO2, fill = sector_agg)) +
  geom_col() + 
  scale_fill_manual(values = pal_industry_sectors, name = "sector") + 
  facet_grid(rows = vars(region), cols = vars(branch), scales = "free_y") + 
  ...
  theme(text=element_text(size=17),
        strip.background = element_blank())
ggsave(paste0(fig_dir, "/CO2_emissions_no_bio_industry_sector_agg_region_ref_comp.png"), width = 10, height = 30)
...
ggplot(nonCO2_emis_sector_total_CO2eq_industry_agg_total %>% filter(scenario == ref_scenario_name & year >= 2005),
  ...
  scale_linetype_manual(values = c("dashed", "solid")) + 
  facet_wrap(~region, nrow = 4, scales = "free_y") + 
  labs(x = "", y = "Mt CO2e", title = "Total industry non-CO2 emissions, reference scenario") + 
  theme_bw() + 
  theme(text=element_text(size=22),
        strip.background = element_blank())
...
ggplot(nonCO2_emis_sector_total_CO2eq_industry_agg_total_global %>% filter(scenario == ref_scenario_name & year >= 2005),
  ...
  geom_line(size = 1) + 
  scale_linetype_manual(values = c("dashed", "solid")) + 
  labs(x = "", y = "Mt CO2e", title = "Global total industry non-CO2 emissions, reference scenario") + 
  theme_bw() + 
  theme(text=element_text(size=17),
        strip.background = element_blank())
...
ggplot(industry_water_withdrawals %>% filter(scenario == ref_scenario_name & year >= 2005 & !grepl("H2", sector)),
  ...
  theme(text=element_text(size=22),
        strip.background = element_blank())
ggsave(paste0(fig_dir, "/industry_water_withdrawals_ref_comp.png"), width = 10, height = 30)
...
```{r, message = FALSE, warning = FALSE}
ggplot(industry_final_en_sector_agg_total %>% filter(year >= 2005),
...
ggplot(industry_final_en_sector_agg_total_global %>% filter(year >= 2005),
       aes(x = year, y = value, linetype = branch, color = scenario)) +
  geom_line(size = 1) + 
  scale_linetype_manual(values = c("dashed", "solid")) + 
  scale_color_manual(values = pal_scenarios) + 
  ...
  theme(text=element_text(size=17),
        strip.background = element_blank())
ggsave(paste0(fig_dir, "/total_industry_energy_use_scenario_branch_comp_global.png"), width = 10, height = 8)

ggplot(industry_final_en_fuel_global %>% 
         group_by(scenario, branch, input_edit, Units, year) %>%
         summarize(value = sum(value)) %>%
         ungroup() %>%
         filter(year >= 2005),
  ...
  facet_wrap(~scenario, ncol = 7, scale = "free_y") +
  labs(x = "", y = "EJ", title = "Global industry final energy use by fuel") +
  theme_bw() +
  theme(text=element_text(size=17),
        strip.background = element_blank())
ggsave(paste0(fig_dir, "/industry_final_en_fuel_scenario_branch_comp_global.png"), width = 22, height = 12)
```
