...
database_name <- "database_basexdb_10-30-23"
run_dir <- "C:/Users/spei632/Documents/GCAM_industry/food_processing/initial_breakout_results/data/food_processing_run_10-30-23"
run_name <- "food_processing_run_10-30-23"
...
# OLD CODE - from before when the global non-energy costs for the overall food processing sector had been estimated
...
# get total food processing overall costs
food_pro_costs_overall <- food_pro_costs_tech %>%
  filter(sector == "food processing") %>%
  left_join(food_demand_total %>%
              ungroup() %>%
              dplyr::select(scenario, region, year, Pcal = value)) %>%
  mutate(total_bil_1975USD = Pcal * 10^9 * value / (10^9), # convert Pcal to Mcal
         total_bil_2015USD = total_bil_1975USD * gcamdata::gdp_deflator(2015, 1975))

# plot
ggplot(food_pro_costs_overall %>% filter(year >= 2005),
       aes(x = year, y = value)) + 
  geom_line(size = 1) + 
  facet_wrap(~region, nrow = 4) + 
  labs(x = "", y = "1975$/Mcal", title = paste0("Food processing costs, ", scenario_sel_plot_name)) + 
  theme_bw() + 
  theme(text=element_text(size=22),
        strip.background = element_blank())
ggsave(paste0(fig_dir, "/food_pro_overall_costs_", scenario_sel_save_name, ".png"), width = 25, height = 20)
...
# get total costs separating out food and non-food inputs
FoodProcessing_GTAP_total_costs_sep_food_inputs <- CostShare_FoodProcessing_GTAP_agg %>%
  mutate(cost_type = if_else(input_AGG %in% c("PrimaryAg", "Proc_Food"), "food_GTAP_bil", "food_pro_GTAP_bil")) %>%
  group_by(year, output, region_GCAM, cost_type) %>%
  summarize(cost_bil = sum(cost_bil)) %>%
  ungroup() %>%
  pivot_wider(names_from = cost_type, values_from = cost_bil)
...
# compare the costs to GTAP data (note that total costs should match on a global level but will differ regionally since the non-energy cost value used in GCAM was calculated globally)
costs_comp_GCAM_GTAP <- food_pro_costs_overall %>%
  filter(year == 2015) %>%
  dplyr::select(scenario, region, Pcal, food_pro_cost_bil_2015USD = total_bil_2015USD) %>%
  left_join(ag_regional_total_costs_food_agg %>%
              filter(year == 2015) %>%
              dplyr::select(scenario, region, food_cost_bil_2015USD = total_cost_bil_2015USD)) %>%
  left_join(FoodProcessing_GTAP_total_costs_sep_food_inputs %>%
              filter(output == "FoodProduct" & year == 2014) %>%
              dplyr::select(region = region_GCAM, food_GTAP_bil, food_pro_GTAP_bil)) %>%
  mutate(perc_dif_food_pro_costs = (food_pro_cost_bil_2015USD - food_pro_GTAP_bil) * 100 / food_pro_GTAP_bil,
         perc_dif_food_costs = (food_cost_bil_2015USD - food_GTAP_bil) * 100 / food_GTAP_bil)

# check global sums
costs_comp_GCAM_GTAP_global <- costs_comp_GCAM_GTAP %>%
  summarize(Pcal = sum(Pcal),
            food_pro_cost_bil_2015USD = sum(food_pro_cost_bil_2015USD),
            food_cost_bil_2015USD = sum(food_cost_bil_2015USD),
            food_GTAP_bil = sum(food_GTAP_bil),
            food_pro_GTAP_bil = sum(food_pro_GTAP_bil))

# plot
ggplot(costs_comp_GCAM_GTAP %>%
         dplyr::select(region, food_pro_GCAM = food_pro_cost_bil_2015USD, food_GCAM = food_cost_bil_2015USD,
                       food_GTAP = food_GTAP_bil, food_pro_GTAP = food_pro_GTAP_bil) %>% 
         pivot_longer(-region, names_to = "cost_type", values_to = "value"),
       aes(x = region, y = value, fill = cost_type)) + 
  geom_col(position = "dodge") + 
  scale_fill_manual(values = c("darkblue", "darkgreen", "royalblue", "mediumseagreen")) + 
  labs(x = "", y = "Billion 2015 USD", title = paste0("Food processing and food costs, 2015/2014, ", scenario_sel_plot_name)) + 
  theme_bw() + 
  theme(text=element_text(size=22),
        strip.background = element_blank(),
        axis.text.x = element_text(angle=75, vjust = 1, hjust = 1))
ggsave(paste0(fig_dir, "/food_pro_food_costs_comp_", scenario_sel_save_name, ".png"), width = 15, height = 8)

ggplot(costs_comp_GCAM_GTAP %>%
         dplyr::select(region, food_pro_GCAM = food_pro_cost_bil_2015USD, food_pro_GTAP = food_pro_GTAP_bil) %>% 
         pivot_longer(-region, names_to = "cost_type", values_to = "value"),
       aes(x = region, y = value, fill = cost_type)) + 
  geom_col(position = "dodge") + 
  ...
  theme(text=element_text(size=22),
        strip.background = element_blank(),
        axis.text.x = element_text(angle=75, vjust = 1, hjust = 1))
ggsave(paste0(fig_dir, "/food_pro_costs_comp_", scenario_sel_save_name, ".png"), width = 15, height = 8)
```
